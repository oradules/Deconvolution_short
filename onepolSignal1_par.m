...
function [Sum_signals] = sumSignal(position,FreqEchSimu, FreqEchImg, TaillePreMarq, ...
...
%%%%%%% compute signal from positions 
    ...
    ximage = (1:frame_num)/FreqEchImg*Polym_speed; %%%% frame positions in bp
        ...
        xpos = position/FreqEchSimu*Polym_speed-(TaillePreMarq+TailleSeqMarq+TaillePostMarq); %%%% polymerase start in bp 
        
        ypos = ximage - xpos - TaillePreMarq; %%% frame position relative to signal raise
        
        ind = find(ypos > 0 & ypos < TailleSeqMarq + TaillePostMarq); 
        
        Sum_signals(ind) = Signal_par(ypos(ind),Intensity_for_1_Polym,TailleSeqMarq);
