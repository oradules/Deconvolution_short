...
%%%%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [positions,Min_fit]=optimize_local_modified(target,guess,num_possible_poly,FreqEchSimu, FreqEchImg, TaillePreMarq, ...
...
%%%%% pattern perturbation to search local minimum
	...
	for posi_i =1:length(positions)
	    ...
	    for j = -shift_window:shift_window
	        ...
	        if displaced <= 0 || displaced > num_possible_poly || ~isempty(find(positions<=displaced & positions >=displaced, 1))
	             continue
	        end               
	        new_pos(posi_i)=displaced;
	        fitness=GD_y_fitness(new_pos);
	        if fitness<Min_fit
	            positions(posi_i) = displaced;
	            Min_fit = fitness;
	        end             
	    end
	end
