...
class readDataInFile:
    def __init__(self,DataFilePath,outputFolder,readDataPath):
        PyFilePath = os.path.join(outputFolder,readDataPath)
    ...
    def read_data(self,data_i):
        ...
        DataExp =pd.read_excel(DataFileName,usecols=np.arange(4,500),skiprows=1).to_numpy() #extracting the data from the excel sheet
        ...
        n=DataExp.shape
        fn=self.file_name_list[data_i].replace('_','').replace('.xlsx','')
        ...
        fname=self.PyFilePath+'data_'+fn+'.npz'
